      ...
      <Card className="mb-4">
        <CardContent className="p-4">
          ...
          <div className="mb-4">
            ...
            <p>{selectedOrder?.customer?.fullName || 'Walk-in Customer'}</p>
            ...
            <TableBody>
              {items.map(item => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">${(item.product?.sellingPrice || 0)?.toFixed(2)}</TableCell>
                  <TableCell className="text-right">${((item.product?.sellingPrice || 0) * (item.quantity || 0)).toFixed(2)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
